...
> [!NOTE]
> 本書の例は Ruby 風の構文を使いますが、説明を簡単にするため、名前解決の規則は「入れ子の静的スコープ」だけを持つ単純化した言語（いわば MiniRuby）として扱います。実際の Ruby には、裸の識別子がローカル変数参照になるかメソッド呼び出しになるかは同じスコープに代入が現れているかで決まる、`def` の中から外側のローカル変数は見えない、トップレベルの `x = 10` はグローバル変数ではなくトップレベルのローカル変数になる、といった独特の規則があります。本物の Ruby の名前解決はそれ自体が一冊分のテーマです。
...
> [!NOTE]
> 対義語は **動的スコープ（dynamic scope）** で、「実行時の呼び出しの連なり」で名前の指す先が決まります。初期の Lisp などが採用していましたが、プログラムを追いにくいため、現代の多くの言語では**通常の変数名の解決**は静的スコープに基づきます。ただし、Common Lisp の special variable、Perl の `local`、各言語のスレッドローカル変数のように、実行時の動的な文脈に依存する仕組みを別途持つ言語は今もあります。本書では通常の変数名解決に話を絞り、以後、特に断らなければ静的スコープを前提とします。
...
class Resolver < Visitor
  ...
  def enter_scope
    outer = @scope
    @scope = Scope.new(outer)    # 内側スコープを積む
    yield
  ensure
    @scope = outer               # 途中で例外が起きても必ず外側へ戻す
  end
end
```
...
これが名前解析の**得られる結果**です。すべての名前の使用が、対応する宣言に結びついた装飾済み AST。教科書的な実装の詳細は[](#cite:appel1998)や[](#cite:cooper2011)が詳しく扱っています。
...
（断っておくと、これは「宣言を先に集める」言語を想定した MiniRuby の例です。実際の Ruby では `def` も上から順に実行時に評価されるため、このコードは `greet` が未定義のまま呼ばれて失敗します。C や Java のトップレベル関数・メソッドが、こうした前方参照を許す言語の代表例です。）
